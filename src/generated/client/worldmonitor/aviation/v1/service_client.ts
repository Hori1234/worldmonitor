...
export interface GetOpenSkyRadarRequest {
  lamin?: number;
  lomin?: number;
  lamax?: number;
  lomax?: number;
...
export interface GetOpenSkyRadarResponse {
  time: number;
  states: RadarFlightState[];
}

export interface RadarFlightState {
  icao24: string;
  callsign: string;
...
export class AviationServiceClient {
  ...
  async listAirportDelays(req: ListAirportDelaysRequest, options?: AviationServiceCallOptions): Promise<ListAirportDelaysResponse> {
    ...
    if (req.region != null && req.region !== null) params.set("region", String(req.region));
    if (req.minSeverity != null && req.minSeverity !== null) params.set("min_severity", String(req.minSeverity));
  ...
  async getOpenSkyRadar(req: GetOpenSkyRadarRequest, options?: AviationServiceCallOptions): Promise<GetOpenSkyRadarResponse> {
    let path = "/api/aviation/v1/get-open-sky-radar";
    const params = new URLSearchParams();
    if (req.lamin != null && req.lamin !== 0) params.set("lamin", String(req.lamin));
    if (req.lomin != null && req.lomin !== 0) params.set("lomin", String(req.lomin));
    if (req.lamax != null && req.lamax !== 0) params.set("lamax", String(req.lamax));
    if (req.lomax != null && req.lomax !== 0) params.set("lomax", String(req.lomax));
    const url = this.baseURL + path + (params.toString() ? "?" + params.toString() : "");
    ...
    return await resp.json() as GetOpenSkyRadarResponse;
  }
