...
export interface AviationServiceHandler {
  ...
  getOpenSkyRadar(ctx: ServerContext, req: GetOpenSkyRadarRequest): Promise<GetOpenSkyRadarResponse>;
}
...
export function createAviationServiceRoutes(
  ...
  return [
    ...
    {
      method: "GET",
      path: "/api/aviation/v1/get-open-sky-radar",
      handler: async (req: Request): Promise<Response> => {
        try {
          const pathParams: Record<string, string> = {};
          const url = new URL(req.url, "http://localhost");
          const params = url.searchParams;
          const body: GetOpenSkyRadarRequest = {
            lamin: Number(params.get("lamin") ?? "0"),
            lomin: Number(params.get("lomin") ?? "0"),
            lamax: Number(params.get("lamax") ?? "0"),
            lomax: Number(params.get("lomax") ?? "0"),
          };
          if (options?.validateRequest) {
            const bodyViolations = options.validateRequest("getOpenSkyRadar", body);
            if (bodyViolations) {
              throw new ValidationError(bodyViolations);
            }
          }
          ...
          const result = await handler.getOpenSkyRadar(ctx, body);
          return new Response(JSON.stringify(result as GetOpenSkyRadarResponse), {
            status: 200,
            headers: { "Content-Type": "application/json" },
          });
        } catch (err: unknown) {
